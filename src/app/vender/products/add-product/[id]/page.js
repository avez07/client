...
const AddInfo = ({ params }) => {
   ...
   const calculateDerivedValues = (index) => {
      ...
      const CalPrice = parseInt(price) - (parseInt(discount) || 0)
...
console.log(productPermit.VariantCheck)
   ...
   return (
      <>
         ...
         <Row md={1} className="g-2">
            <div className="add-product-head  rounded-2  py-1">
               <ul>
                  ...
                  {productPermit?.VariantCheck ? (<li className={`${pagecount == 3 ? 'active' : ''}`} onClick={(e) => setPagecount(3)}>{PageValidation[2] == 1 && (<span className="text-danger pe-1"><FaInfoCircle /></span>)}Variant</li>) : (<li className={`${pagecount == 3 ? 'active' : ''}`} onClick={(e) => setPagecount(3)}>{PageValidation[2] == 1 && (<span className="text-danger pe-1"><FaInfoCircle /></span>)}Offer</li>)}
                        ...
                        <Col md={6} key={`datas-${items.name}`}>
                           ...
                           {items.type == 'DropDwon' ? (
                              <Select
                                 ...
                                 menuPlacement="bottom"
                                 className="categoryName"
                                 value={{ value: TableData[0][items.name] || '', label: TableData[0][items.name] || '' }}
                                 onChange={(e) =>
                                    setTableData((prev) =>
                                       prev.map((item, idx) =>
                                          idx === 0
                              ...
                              <>
                                 <Form.Control value={TableData?.[0]?.[items.name] || ''} onChange={(e) =>
                                    setTableData((prev) =>
                                       prev.map((item, idx) =>
                                          idx === 0
         ...
         {pagecount == 4 && (
            <div>
               {!productPermit?.VariantCheck || variantOption.filter(elements => valueToFind.includes(elements)).length == 0 ? (
                  <Row className="g-3">
